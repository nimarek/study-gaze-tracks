...
def build_mask(prep_dir, left_roi="ctx-lh-fusiform", right_roi="ctx-rh-fusiform"):
    # get look up table
    lut_df = pd.read_csv(os.path.join(prep_dir, "desc-aparcaseg_dseg.tsv"), sep="\t")

    # get inf. temporal masks
    matches = lut_df[lut_df["name"] == left_roi]
    left_index = matches["index"].values[0]
    matches = lut_df[lut_df["name"] == right_roi]
    right_index = matches["index"].values[0]

    # load aparc and create mask
    aparc_fpath = os.path.join(prep_dir, f"sub-{subject_id}/ses-movie/func/sub-{subject_id}_ses-movie_task-movie_run-{run_id}_space-T1w_desc-aparcaseg_dseg.nii.gz")
    aparc_data = nib.load(aparc_fpath).get_fdata()
    affine = nib.load(aparc_fpath).affine 

    surface_mask = np.zeros_like(aparc_data, dtype=bool)
    surface_mask[aparc_data == float(left_index)] = True
    surface_mask[aparc_data == float(right_index)] = True
    return nib.nifti1.Nifti1Image(surface_mask*1.0, affine=affine)
...
def prep_models_and_args(subject_id=None, run_id=None, task_label=None, fwhm=None, bidsroot=None, 
    ...
    models, models_run_imgs, \
            ...
            models_confounds = first_level_from_bids(bidsroot, 
                                                     ...
                                                     mask_img=build_mask(prep_dir=fmriprep_dir), 
                                                     smoothing_fwhm=fwhm,
                                                     high_pass=0.008,
                                                     slice_time_ref=0.4854998519225452,
    ...
    for midx in range(len(models)):
        for sx, stim in enumerate(stim_list):
            ...
            for rx in range(len(confounds_ltd)):
                ...
                save_path = os.path.join("/home", "data", "study_gaze_tracks", "scratch", "local_code", "derivatives", f"lss_space-roi_condition-face-tracks", f"sub-{subject_id}")
                if not os.path.exists(save_path):
                    os.makedirs(save_path, exist_ok=True)

                sub_fname = f"sub-{subject_id}_run-{run_id}_roi-inf-temp_contrast-{contrast_desc}"
                statmap_fpath = os.path.join(save_path, sub_fname + "_map-beta.nii.gz")
                ...
                #                          bg_img=f"/home/data/study_gaze_tracks/studyforrest-data-phase2/derivatives/fmriprep_mni/sub-{subject_id}/ses-movie/anat/sub-{subject_id}_ses-movie_desc-preproc_T1w.nii.gz",
...
stim_list, models, models_run_imgs, \
    ...
    conf_keep_list = prep_models_and_args(subject_id, run_id, task_label, fwhm, bidsroot, 
                                          fmriprep_dir, 
